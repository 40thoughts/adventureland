...
function reset_topleft() {
    ...
    if (ctarget && ctarget.type == "monster" && last_target_cid != ctarget.cid) {
        ...
        var d = [{
            ...
            value: k + "/" + e,
            cursed: h.s.cursed,
            stunned: !h.attack && h.s.stunned,
            poisoned: !h.attack && h.s.poisoned
        }, {
            name: "XP",
            color: "green",
            value: f
        ...
        if (c.abilities) {
            for (var b in c.abilities) {
                ...
                d.push({
                    name: "ABILITY",
                    color: "#FC5F39",
                    value: G.skills[b].name.toUpperCase()
                })
            }
        }
        if (h.target) {
        ...
        if (ctarget && ctarget.npc) {
            ...
            if (ctarget && ctarget.type == "character" && last_target_cid != ctarget.cid) {
                var j = ctarget;
                var d = [{
                    name: j.role && j.role.toUpperCase() || "NAME",
                    color: j.role && "#E14F8B" || "gray",
                    value: j.name
                }, {
                    name: "LEVEL",
                    color: "orange",
                    ...
                    name: (j.ctype == "priest" && "HEAL" || "ATT"),
                    color: "green",
                    value: round(j.attack),
                    cursed: j.s.cursed
                }, {
                    name: "ATTSPD",
                    color: "gray",
                    value: round(j.frequency * 100),
...
function adopt_soft_properties(b, d) {
    if (b.me) {
        b.stats = {};
        ["str", "dex", "int", "vit"].forEach(function(f) {
            b.stats[f] = d[f]
        });
    ...
    if (b.type == "character" && b.skin && b.skin != d.skin && !b.rip) {
        if (!XYWH[d.skin]) {
            d.skin = "naked"
        }
        b.skin = d.skin;
        new_sprite(b, "full", "renew");
        restore_dimensions(b)
    }
    for (prop in d) {
        if (asp_skip[prop]) {
            continue
    ...
    if (b.slots) {
        b.g10 = b.g9 = b.g8 = undefined;
        for (var e in b.slots) {
            if ((e == "chest" || e == "mainhand") && b.slots[e]) {
                if (b.slots[e].level >= 10) {
                    b.g10 = true
                }
                if (b.slots[e].level == 9) {
                    b.g9 = true
                }
                if (b.slots[e].level == 8) {
                    b.g8 = true
                }
            }
        }
    ...
    socket.on("hit", function(data) {
        ...
        if (1) {
            ...
            if (data.heal !== undefined) {
                ...
                if (entity && entity.me) {
                    call_code_function("trigger_character_event", "heal", attack_data)
                }
                if (owner && owner.me && data.source == "heal") {
                    resolve_deferred("heal", attack_data)
                }
            } else {
                call_code_function("trigger_event", "attack", attack_data);
                if (entity && entity.me) {
                    call_code_function("trigger_character_event", "attack", attack_data)
                }
                if (owner && owner.me && data.source == "attack") {
                    resolve_deferred("attack", attack_data)
                }
            }
        }
        draw_trigger(function() {
...
function cosmetics_logic(g) {
    ...
    if (g.rip) {
        ...
        for (var b in g.cx) {
            var e = g.cx[b];
            if (!e || ["stone"].includes(b)) {
                continue
            }
            if (a == "full" && ["head", "hair"].includes(b)) {
                continue
            }
            if (a == "armor" && b == "head") {
                f.push("mbody1a")
    ...
    j += G.cosmetics[g.skin + "_height"] || 0;
    f.forEach(function(k) {
        if (T[k] == "head") {
            d += G.cosmetics[k + "_height"] || 0
        }
        if (T[k] == "hair") {
            h += G.cosmetics[k + "_height"] || 0
        }
    });
    if (g.rip) {
        j = (G.cosmetics[(g.rip === true && "gravestone" || g.rip) + "_height"] || 0) - G.cosmetics.default_hat_place - 8;
        d = 0;
        h = 0
    }
    f.forEach(function(m) {
        ...
        if (!g.cxc[m]) {
            ...
            if (m == g.skin + "copy") {
                ...
                if (m == "bg") {
                    ...
                    if (["hat", "hair", "head", "face"].includes(T[m])) {
                        l = new_sprite(m, T[m]);
                        l.anchor.set(0.5, 1);
                        g.addChild(l);
                        g.cxc[m] = l
                        ...
                        if (T[m] == "armor") {
                            ...
                            if (T[m] == "skin") {
                                ...
                                if (T[m] == "wings") {
                                    ...
                                    if (T[m] == "s_wings") {
                                        l = new_sprite(m, "s_wings");
                                        l.anchor.set(0.5, 1);
                                        g.addChild(l);
                                        g.cxc[m] = l
                                    } else {
                                        console.log("Invalid cosmetics: " + m);
                                        return
                                    }
                                }
                            }
        ...
        if (l.stype == "copy") {
            if (g.j !== undefined) {
                l.texture = textures[g.skin][g.i][g.j]
            }
            l.zy = 0
        }
        if (l.stype == "bg") {
            ...
            if (l.stype == "head") {
                l.y_disp = -(G.cosmetics.default_head_place + d + j) + head_y;
                l.zy = ZEPS;
                var k = 0;
                if (g.i == 0) {
                    l.y = l.y_disp + 1
                ...
                if (l.stype == "hair") {
                    l.y_disp = -(G.cosmetics.default_hair_place + j + d + (G.cosmetics[m + "_place"] || 0)) + head_y;
                    var k = 0;
                    l.zy = 2 * ZEPS;
                    if (g.i == 0) {
                        l.y = l.y_disp + 1
                    ...
                    if (l.stype == "hat") {
                        l.y_disp = -(G.cosmetics.default_hat_place + j + d + h + (G.cosmetics[m + "_place"] || 0)) + head_y;
                        var k = 0;
                        l.zy = 3 * ZEPS;
                        if (g.i == 0) {
                            l.y = l.y_disp + 1
                        ...
                        if (l.stype == "face") {
                            l.y_disp = -(j + d + (G.cosmetics[m + "_place"] || 0)) + head_y - 7;
                            l.y = l.y_disp;
                            var k = 0;
                            l.zy = 2.5 * ZEPS;
                            if (g.i == 0) {
                            ...
                            if (l.stype == "skin") {
                                ...
                                if (g.j !== undefined) {
                                    set_texture(l, g.i, g.j)
                                }
                            } else {
                                if (l.stype == "armor") {
                                    if (!l.texture.frame.p) {
                                        l.texture.frame.p = true;
                                        l.texture.frame.height -= 8;
                                        l.texture._updateUvs();
                                        l.y = -8
                                    }
                                    if (g.j == 0) {
                                        l.zy = 2 * ZEPS
...
function load_game(a) {
    loader.load(function(m, d) {
        ...
        for (name in G.sprites) {
            ...
            if (in_arr(g.type, ["v_animation", "head", "hair", "hat", "s_wings", "face"])) {
                b = 1,
                k = g.type
            }
            if (in_arr(g.type, ["wings", "body", "armor", "skin"])) {
                k = g.type
            }
